...
run_hyper() {
  ...
  echo "[$start] >>> Running $script with config $hyper_file"
  {
    echo "[$start] >>> START $script with $hyper_file"
    time "$runner" "$script" -p "$hyper_file"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] >>> FINISH $script"
  } >"$log" 2>&1
...
run python3 code/protacs_16_gbdt_train_retrain_wide.py 
...
run python3 code/protacs_24_rna_gex.py
