...
- data/PCA_GSEA_C5.csv

Requirements
------------
Python >= 3.8
Dependencies: pandas, numpy, matplotlib, seaborn, scikit-learn, scipy, gseapy
Custom: device_summarystatistics (in same directory)
...
output_scaled = (output - output.mean()) / output.std() # Z-scale
...
def gsea_filter(x):
    """
    Filter GSEA results for top up- and down-regulated pathways.

    Parameters
    ----------
    x : pandas.DataFrame
        GSEA results containing at least 'NES' and 'FDR q-val' columns.

    Returns
    -------
    pandas.DataFrame
        Subset of pathways with FDR < cutoff, including the top `top_pathways`
        lowest and highest NES entries.
    """
